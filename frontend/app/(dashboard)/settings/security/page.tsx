...
export default function SecuritySettingsPage() {
  ...
  const [confirmSavedCodes, setConfirmSavedCodes] =
    React.useState<boolean>(false);
    ...
    try {
      ...
      toast.error(
        (err as { message?: string }).message ||
          "Could not initialize 2FA configuration.",
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
          <div className="space-y-1">
            <CardTitle className="text-md font-bold flex items-center gap-2">
              <Smartphone className="h-4 w-4 text-indigo-600" /> Two-Factor
              Authentication (2FA)
            </CardTitle>
            <CardDescription>
              Secure your ecosystem account using standard time-based tokens
              (TOTP).
            </CardDescription>
          </div>
          <div>
            {is2FAEnabled ? (
              ...
              <Badge
                variant="secondary"
                className="bg-slate-100 hover:bg-slate-200 border-none text-slate-600 font-semibold flex items-center gap-1 px-2.5 py-1"
              >
        ...
        <CardContent className="text-sm text-muted-foreground">
          {is2FAEnabled ? (
            <p>
              Your account context is verified using a hardware or software
              authenticator app before granting entry credentials.
            </p>
          ) : (
            <p>
              Multi-factor verification is not configured yet. We recommend
              enabling 2FA to protect your transactions and balances.
            </p>
          )}
        </CardContent>
      ...
      <Dialog
        open={setupModalOpen}
        onOpenChange={(open) =>
          !loading && step !== 3 && setSetupModalOpen(open)
        }
      >
        <DialogContent className="sm:max-w-[480px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Enhance your account security using a 3-step setup framework.
            </DialogDescription>
          </DialogHeader>
          ...
          {step === 1 && setupData && (
            <div className="space-y-4">
              <div className="text-xs text-slate-600 font-semibold">
                Step 1: Scan this QR matrix using your mobile authenticator app
                (Google Authenticator, Duo, or 1Password).
              </div>
              <div className="flex justify-center p-3 bg-white border rounded-xl w-fit mx-auto shadow-inner">
                <img
                  src={setupData.qrCodeDataUrl}
                  alt="2FA Setup QR Matrix"
                  className="w-44 h-44"
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">
                  Manual Account Key Configuration
                </label>
          ...
          {step === 2 && (
            <form onSubmit={handleVerifyEnable} className="space-y-4">
              <div className="text-xs text-slate-600 font-semibold">
                Step 2: Enter the 6-digit confirmation token visible inside your
                app.
              </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2 text-amber-600 bg-amber-50 border border-amber-100 rounded-lg p-3">
                ...
                <div className="text-[11px] leading-relaxed font-medium">
                  <strong>Warning:</strong> Keep these emergency backup recovery
                  phrases safe. They will not be displayed again.
                </div>
              </div>
              ...
              <div className="flex items-start space-x-2 pt-1">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setConfirmSavedCodes(!!checked)}
                />
                <label
                  htmlFor="confirm-save"
                  className="text-xs leading-none text-slate-600 font-medium select-none cursor-pointer"
                >
      ...
      <Dialog
        ...
        onOpenChange={(open) => !loading && setDisableModalOpen(open)}
      >
        <DialogContent className="sm:max-w-[400px]">
          <DialogHeader>
            ...
            <DialogDescription>
              To disable 2FA protections, confirm your primary identity profile
              parameters below.
            </DialogDescription>
          </DialogHeader>
