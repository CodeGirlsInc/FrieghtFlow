...
export class PaymentVerificationService {
  ...
  verifyStellarSignature(payload: Record<string, any>, signature: string): boolean {
    try {
      const webhookSecret = this.configService.get<string>("STELLAR_WEBHOOK_SECRET")

      if (!webhookSecret) {
        this.logger.warn("STELLAR_WEBHOOK_SECRET is not configured")
        return false
      }
      ...
      this.logger.error(`Error verifying Stellar signature: ${error.message}`, error.stack)
      return false
    }
  }
