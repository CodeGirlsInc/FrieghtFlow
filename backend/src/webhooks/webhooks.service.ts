...
export class WebhooksService {
  ...
  async processWebhook(
    ...
    // Validate webhook
    const validationResult = await this.validateWebhook(
      source,
      rawPayload,
      headers,
      payload,
    ...
    if (validationResult.isValid) {
      ...
      this.logger.warn(
        `Webhook validation failed for ${source}: ${validationResult.error}`,
      );
    }
  ...
  async validateWebhook(
    ...
    try {
      ...
      const { eventType, eventId } = validator.extractEventInfo(
        headers,
        payload,
      );
      ...
      if (sourceConfig.validateSignature && sourceConfig.secretKey) {
        ...
        const isSignatureValid = validator.validateSignature(
          rawPayload,
          signature,
  ...
  private async emitInternalEvent(webhookEvent: WebhookEvent): Promise<void> {
    try {
      ...
      this.logger.error(
        `Failed to emit internal event for webhook ${webhookEvent.id}`,
        error.stack,
      );
      await this.updateEventStatus(
        webhookEvent.id,
  ...
  async findAll(
    filterDto: FilterWebhookEventsDto,
  ...
  async getWebhookStats(source?: string): Promise<{
    ...
    const queryBuilder =
      this.webhookEventRepository.createQueryBuilder('event');
    ...
    const recentActivityResult = await queryBuilder
      .clone()
    ...
    return {
      ...
      recentActivity: recentActivityResult.lastActivity
        ? new Date(recentActivityResult.lastActivity)
        : null,
    };
  }
