...
describe("WebhooksModule (Integration)", () => {
  ...
  describe("Webhook Processing", () => {
    it("should process a GitHub webhook", async () => {
      ...
      const response = await request(app.getHttpServer()).post("/webhook/github").set(headers).send(payload).expect(200)
    ...
    it("should reject webhook from unknown source", async () => {
      ...
      await request(app.getHttpServer()).post("/webhook/unknown-source").send(payload).expect(400)
    })
  ...
  describe("Webhook Events API", () => {
    ...
    it("should get all webhook events", async () => {
      const response = await request(app.getHttpServer()).get("/webhook/events").expect(200)
    ...
    it("should filter webhook events by source", async () => {
      const response = await request(app.getHttpServer()).get("/webhook/events?source=generic").expect(200)
    ...
    it("should get a specific webhook event", async () => {
      const response = await request(app.getHttpServer()).get(`/webhook/events/${webhookEventId}`).expect(200)
    ...
    it("should return 400 for non-existent webhook event", async () => {
      ...
      await request(app.getHttpServer()).get(`/webhook/events/${nonExistentId}`).expect(400)
    })

    it("should get webhook statistics", async () => {
      const response = await request(app.getHttpServer()).get("/webhook/stats").expect(200)
    ...
    it("should paginate webhook events", async () => {
      ...
      const response = await request(app.getHttpServer()).get("/webhook/events?page=1&limit=3").expect(200)
  ...
  describe("Event Filtering", () => {
    it("should filter events by date range", async () => {
      ...
      const response = await request(app.getHttpServer())
        .get(`/webhook/events?startDate=${startDate.toISOString()}&endDate=${endDate.toISOString()}`)
        .expect(200)
    ...
    it("should filter events by event type", async () => {
      ...
      const response = await request(app.getHttpServer()).get("/webhook/events?eventType=user.deleted").expect(200)
