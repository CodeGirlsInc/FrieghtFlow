...
export class AuditLogService {
  ...
  async createLog(createAuditLogDto: CreateAuditLogDto): Promise<AuditLog> {
    try {
      ...
      this.logger.log(`Audit log created: ${savedLog.action} for user ${savedLog.userId || "system"}`)
      return savedLog
    } catch (error) {
  ...
  async findAll(queryDto: QueryAuditLogDto): Promise<{ logs: AuditLog[]; total: number }> {
    const { limit, offset, action, userId, entityType, entityId, startDate, endDate, search } = queryDto
  ...
  async logUserCreation(userId: string, userEmail: string, userData: any, metadata?: any): Promise<AuditLog> {
  ...
  async logPaymentProcessing(userId: string, paymentId: string, paymentData: any, metadata?: any): Promise<AuditLog> {
    return this.createLog({
