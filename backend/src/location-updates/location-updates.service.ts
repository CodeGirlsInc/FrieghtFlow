...
export class LocationUpdatesService {
  ...
  async recordLocation(
    ...
    if (shipment.status !== ShipmentStatus.IN_TRANSIT)
      throw new BadRequestException('Shipment is not in transit');
    if (shipment.carrierId !== carrierId)
      throw new ForbiddenException(
        'Only the assigned carrier can update location',
      );

    const location = this.locRepo.create({ shipmentId, lat, lng });
    await this.locRepo.save(location);

    const history = await this.locRepo.find({
      where: { shipmentId },
      order: { createdAt: 'ASC' },
    });
    ...
    this.eventEmitter.emit('shipment.location.updated', {
      shipmentId,
      lat,
      lng,
      timestamp,
