    ...
    descriptor.value = async function (...args: any[]) {
      ...
      try {
        ...
        if (options.level !== LogLevel.TRACE) {
          logger.debug?.(
            `Entering ${target.constructor.name}.${propertyName}`,
            {
              ...context,
              args: options.includeArgs
                ? this.sanitizeArgs(args, options.maskArgs)
                : undefined,
            },
          );
        }
        ...
        const message =
          options.message ||
          `${target.constructor.name}.${propertyName} completed`;
        const logLevel =
          options.performance && duration > 1000
            ? LogLevel.WARN
            : LogLevel.DEBUG;
...
export function LogClass(options: LogDecoratorOptions = {}) {
  return <T extends { new (...args: any[]): {} }>(constructor: T) =>
    class extends constructor {
          ...
          prototype[methodName] = function (...args: any[]) {
            ...
            try {
              ...
              if (options.performance && duration > 100) {
                logger.warn?.(
                  `Slow method: ${constructor.name}.${methodName} took ${duration}ms`,
                );
              }
              ...
              const duration = Date.now() - startTime;
              logger.error?.(
                `Method ${constructor.name}.${methodName} failed after ${duration}ms`,
                error,
              );
              throw error;
            }
      ...
      public sanitizeArgs(args: any[], maskFields: string[] = []): any[] {
        return args.map((arg) => {
