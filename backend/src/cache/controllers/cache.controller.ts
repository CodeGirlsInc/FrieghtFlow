...
export class CacheController {
  ...
  async delete(deleteCacheDto: DeleteCacheDto) {
    const deleted = await this.cacheService.del(
      deleteCacheDto.keys,
      deleteCacheDto.namespace,
    );
  ...
  async expire(expireCacheDto: ExpireCacheDto) {
    const result = await this.cacheService.expire(
      expireCacheDto.key,
      expireCacheDto.seconds,
      expireCacheDto.namespace,
    );
    return {
      success: result,
      key: expireCacheDto.key,
      seconds: expireCacheDto.seconds,
    };
  }
