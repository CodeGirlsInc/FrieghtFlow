...
export class DisputesService {
  ...
  async create(userId: string, dto: CreateDisputeDto): Promise<Dispute> {
    ...
    if (shipment.shipperId !== userId && shipment.carrierId !== userId) {
      throw new ForbiddenException('You must be a party to the shipment to open a dispute');
    }

    const active = await this.disputeRepo.findOne({ where: { shipmentId: dto.shipmentId, status: DisputeStatus.OPEN } });
    if (active) throw new BadRequestException('An active dispute already exists for this shipment');

    const dispute = this.disputeRepo.create({ shipmentId: dto.shipmentId, openedById: userId, reason: dto.reason });
    return this.disputeRepo.save(dispute);
  }

  async findOne(id: string, userId: string, role: UserRole): Promise<Dispute> {
    const dispute = await this.disputeRepo.findOne({ where: { id }, relations: ['shipment'] });
    if (!dispute) throw new NotFoundException('Dispute not found');
    if (role !== UserRole.ADMIN && dispute.openedById !== userId && dispute.shipment.shipperId !== userId && dispute.shipment.carrierId !== userId) {
  ...
  async findAllAdmin(query: { page?: number; limit?: number; status?: DisputeStatus }) {
