...
import { type ShippingDocument, DocumentStatus } from "./entities/document.entity"
...
export class FileUploadService {
  ...
  async uploadDocument(
    ...
    const requestValidation = this.fileValidationService.validateUploadRequest(uploadDto, uploadDto.documentType)
    if (!requestValidation.isValid) {
      throw new BadRequestException(`Validation failed: ${requestValidation.errors.join(", ")}`)
    }

    // Validate the file
    const fileValidation = this.fileValidationService.validateFile(file, uploadDto.documentType)
    ...
    try {
      // Store the file
      const storageResult = await this.fileStorageService.storeFile(file, uploadDto.documentType, userId)
  ...
  async findAll(queryDto: QueryDocumentsDto): Promise<{ documents: ShippingDocument[]; total: number }> {
  ...
  async downloadDocument(id: string): Promise<{ buffer: Buffer; document: ShippingDocument }> {
    const document = await this.findOne(id)

    try {
      const buffer = await this.fileStorageService.getFileStream(document.filePath, document.s3Key)
  ...
  async deleteDocument(id: string): Promise<void> {
    ...
    try {
      ...
      await this.fileStorageService.deleteFile(document.filePath, document.s3Key)
  ...
  async updateDocumentStatus(id: string, status: DocumentStatus): Promise<ShippingDocument> {
    const document = await this.findOne(id)
  ...
  async getDocumentsByShipment(shipmentId: string): Promise<ShippingDocument[]> {
