...
export class FileStorageService {
  ...
  async storeFile(file: Express.Multer.File, documentType: string, userId?: string): Promise<StorageResult> {
  ...
  async getFileStream(filePath: string, s3Key?: string): Promise<Buffer> {
    try {
      if (this.useS3 && s3Key) {
        ...
        const localPath = path.join(UPLOAD_CONFIG.LOCAL_UPLOAD_DIR, path.basename(filePath))
  ...
  private generateFileName(originalName: string, documentType: string, userId?: string): string {
