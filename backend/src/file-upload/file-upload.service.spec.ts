...
describe('FileUploadService', () => {
  ...
  beforeEach(async () => {
    ...
    repository = module.get<Repository<ShippingDocument>>(
      getRepositoryToken(ShippingDocument),
    );
  ...
  describe('uploadDocument', () => {
    ...
    it('should successfully upload a document', async () => {
      ...
      expect(fileValidationService.validateUploadRequest).toHaveBeenCalledWith(
        uploadDto,
    ...
    it('should throw BadRequestException for invalid upload request', async () => {
      ...
      await expect(service.uploadDocument(mockFile, uploadDto)).rejects.toThrow(
        BadRequestException,
      );
  ...
  describe('findOne', () => {
    ...
    it('should throw NotFoundException when document not found', async () => {
      ...
      await expect(service.findOne('doc-123')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('downloadDocument', () => {
    it('should return document buffer and metadata', async () => {
      ...
      expect(fileStorageService.getFileStream).toHaveBeenCalledWith(
