...
export interface QuoteEstimate {
  ...
  breakdown: { base: number; distanceFee: number; weightFee: number };
}
...
export class QuotesService {
  ...
  estimate(
    ...
    const zone =
      origin === destination
        ? 'local'
        : origin.slice(0, 2) === destination.slice(0, 2)
          ? 'regional'
          : 'national';
    const rate = ZONE_RATES[zone] ?? 2.0;
    const base = 50,
      distanceFee = rate * 30,
      weightFee = weightKg * 0.5;
    const total = base + distanceFee + weightFee;
    this.logger.log(
      `Quote ${origin}->${destination} ${weightKg}kg zone=${zone} ~$${total.toFixed(2)}`,
    );
    return Promise.resolve({
      minPrice: Math.round(total * 0.9),
      maxPrice: Math.round(total * 1.1),
      currency: 'USD',
      estimatedDays: zone === 'local' ? 1 : zone === 'regional' ? 3 : 7,
      breakdown: { base, distanceFee, weightFee },
    });
  }
}
