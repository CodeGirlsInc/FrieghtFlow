...
export class SystemHealthChecker implements HealthChecker {
  ...
  async check(): Promise<HealthCheckResult> {
    ...
    try {
      ...
      const systemMemoryUsedPercent = (systemMemory.used / systemMemory.total) * 100
      ...
      if (heapUsedPercent > 90 || systemMemoryUsedPercent > 95 || (diskSpace?.usedPercent || 0) > 95) {
        status = HealthStatus.UNHEALTHY
      } else if (heapUsedPercent > 80 || systemMemoryUsedPercent > 85 || (diskSpace?.usedPercent || 0) > 85) {
        status = HealthStatus.DEGRADED
      }

      // Check load average (if available)
      if (loadAverage[0] > os.cpus().length * 2) {
        status = status === HealthStatus.HEALTHY ? HealthStatus.DEGRADED : HealthStatus.UNHEALTHY
      }
  ...
  private async getDiskSpace(): Promise<{ total: string; free: string; used: string; usedPercent: number } | null> {
    try {
