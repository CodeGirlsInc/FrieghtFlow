...
export class StellarEscrowBridgeService {
  ...
  fundEscrow(
    ...
    amount: number,
    tokenAddress: string,
  ): Promise<{ txHash: string; ledgerNumber: number }> {
    if (!shipmentId || amount <= 0) {
      throw new BadGatewayException(
        'Stellar contract call failed: invalid parameters',
      );
    }
    this.logger.log(
      `Funding escrow for shipment ${shipmentId}: ${amount} ${tokenAddress}`,
    );
    return Promise.resolve({
      txHash: 'stellar-tx-' + Date.now(),
      ledgerNumber: 12345,
    });
  }
