...
export class EscrowPaymentService {
  ...
  holdPayment(
    ...
    this.logger.log(
      `Holding ${amount} XLM in escrow for shipment ${shipmentId} (shipper: ${shipperPublicKey})`,
    );
    const hold: EscrowHold = {
      shipmentId,
      amount,
      txHash: `escrow-hold-${Date.now()}`,
      status: 'held',
    };
  ...
  releasePayment(
    ...
    if (!escrow)
      throw new BadRequestException('No escrow found for this shipment');
    this.logger.log(
      `Releasing escrow for shipment ${shipmentId} to carrier ${carrierPublicKey}`,
    );
    escrow.status = 'released';
    return Promise.resolve(escrow);
  }

  refundPayment(shipmentId: string): Promise<EscrowHold> {
    const escrow = this.escrows.get(shipmentId);
    if (!escrow)
      throw new BadRequestException('No escrow found for this shipment');
