...
export class ReputationCalculatorService {
  ...
  async calculateScore(carrierId: string) {
    const reviews = await this.reviewRepo.find({ where: { revieweeId: carrierId } });
    ...
    const completedCount = shipments.filter(s =>
      s.status === ShipmentStatus.COMPLETED || s.status === ShipmentStatus.DELIVERED
    ).length;
    const completionRate = shipments.length > 0 ? completedCount / shipments.length : 0;
    ...
    const compositeScore = Math.min(ratingComponent + punctualityComponent + reliabilityComponent, 1000);

    return { ratingComponent: Math.round(ratingComponent), punctualityComponent: Math.round(punctualityComponent * 100) / 100, reliabilityComponent: Math.round(reliabilityComponent * 100) / 100, compositeScore: Math.round(compositeScore) };
  }
}
