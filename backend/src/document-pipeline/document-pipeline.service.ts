...
export class DocumentPipelineService {
  constructor(
    ...
    private readonly procRepo: Repository<DocumentProcessing>,
  ...
  async enqueue(documentId: string): Promise<DocumentProcessing> {
    ...
    const processing = this.procRepo.create({
      documentId,
  ...
  async process(documentId: string): Promise<DocumentProcessing> {
    let proc = await this.procRepo.findOne({
      where: { documentId },
      order: { createdAt: 'DESC' },
    });
    if (!proc) {
      proc = this.procRepo.create({
        documentId,
        status: ProcessingStatus.PENDING,
      });
    }
