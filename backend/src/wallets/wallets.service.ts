...
export interface LinkedWallet { userId: string; publicKey: string; verified: boolean; linkedAt: Date; }
...
export class WalletsService {
  ...
  async linkWallet(userId: string, publicKey: string, signedChallenge: string): Promise<LinkedWallet> {
    if (!publicKey.startsWith('G') || publicKey.length !== 56) {
      throw new BadRequestException('Invalid Stellar public key');
    }
    if (!signedChallenge) throw new BadRequestException('Signed challenge required for verification');
    this.logger.log(`Linking wallet ${publicKey} for user ${userId}`);
    const wallet: LinkedWallet = { userId, publicKey, verified: true, linkedAt: new Date() };
