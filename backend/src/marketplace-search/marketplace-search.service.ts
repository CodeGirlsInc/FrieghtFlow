...
export class MarketplaceSearchService {
  ...
  async search(query: SearchMarketplaceDto) {
    ...
    if (origin)
      qb.andWhere('s.origin ILIKE :origin', { origin: `%${origin}%` });
    if (destination)
      qb.andWhere('s.destination ILIKE :destination', {
        destination: `%${destination}%`,
      });
    if (minPrice !== undefined)
      qb.andWhere('s.price >= :minPrice', { minPrice });
    if (maxPrice !== undefined)
      qb.andWhere('s.price <= :maxPrice', { maxPrice });
    if (maxWeightKg !== undefined)
      qb.andWhere('s.weightKg <= :maxWeightKg', { maxWeightKg });
    if (cargoCategory)
      qb.andWhere('s.cargoCategory = :cargoCategory', { cargoCategory });
    ...
    qb.orderBy(sortMap[sortBy] ?? 's.createdAt', 'DESC')
      .skip(skip)
      .take(limit);
