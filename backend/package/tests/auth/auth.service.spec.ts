...
describe('AuthService', () => {
  ...
  describe('register', () => {
    it('should register a new user and return tokens', async () => {
      const registerDto = { email: 'test@test.com', password: 'password', firstName: 'Test', lastName: 'User', role: 'user' };
      const user = { id: '1', ...registerDto, passwordHash: 'hashedPassword', refreshToken: null, isActive: true, verificationToken: '', verificationTokenExpiry: new Date() };
    ...
    it('should throw ConflictException if email is already taken', async () => {
      ...
      it('should throw BadRequestException for weak passwords', async () => {
        const registerDto = { email: 'test@test.com', password: '123', firstName: 'Test', lastName: 'User', role: 'user' };
        mockUsersService.create.mockRejectedValue(new BadRequestException('Password is too weak'));
  ...
  describe('login', () => {
    it('should login a user and return tokens', async () => {
        const user = { id: '1', email: 'test@test.com', passwordHash: 'hashedPassword', refreshToken: null, isActive: true, role: 'user' };
        mockJwtService.signAsync.mockResolvedValue('test-token');
      ...
      it('should throw UnauthorizedException for wrong password', async () => {
        ...
        await expect(authService.validateUser('test@test.com', 'wrongpassword')).resolves.toBeNull();
      });
  
      it('should throw ForbiddenException for unverified email', async () => {
        const user = { id: '1', email: 'test@test.com', passwordHash: 'hashedPassword', refreshToken: null, isActive: false, role: 'user' };
        
        await expect(authService.login(user as any)).rejects.toThrow(UnauthorizedException);
      });
  });

  describe('refreshToken', () => {
    it('should refresh tokens successfully', async () => {
        const user = { id: '1', email: 'test@test.com', passwordHash: 'hashedPassword', refreshToken: 'hashed-refresh-token', isActive: true, role: 'user' };
      ...
      it('should throw UnauthorizedException for expired token', async () => {
        ...
        await expect(authService.refresh('1', 'raw-refresh-token')).rejects.toThrow(UnauthorizedException);
      });
